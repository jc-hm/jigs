...
- **`src/services/ai/agent.ts`** — Sonnet multi-turn tool-use agent for file operations. Max 10 rounds. Tools: read_file, write_file, delete_file, move_file, list_files. `executeFileOperations` is an async generator that yields `AgentEvent`s (`tool` per completed tool call, `complete` at the end) — the route relays these as SSE so the UI gets progress before the loop finishes.
