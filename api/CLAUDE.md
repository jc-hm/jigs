...
- **`src/services/ai/agent.ts`** — Sonnet multi-turn tool-use agent for file operations. Max 25 rounds (each round = one Bedrock call). Tools: read_file, write_file, delete_file, move_file, list_files, create_folder. `create_folder` calls `ops.mkdir` which writes a zero-byte placeholder key ending in `/`; `ls`/`lsRecursive` both filter those placeholders so they stay invisible to users and to Claude's file-list snapshot, but the folder itself shows up via CommonPrefixes. `executeFileOperations` is an async generator that yields `AgentEvent`s (`tool` per completed tool call, `complete` at the end) — the route relays these as SSE so the UI gets progress before the loop finishes. The system prompt instructs Claude to batch independent tool calls in a single turn (parallel tool use) so bulk operations like "create 30 templates" finish in 2-3 rounds instead of exhausting the budget one file at a time.
- **`src/lib/sse.ts`** — `sseLine(event)` is the canonical SSE wire-format helper (`data: {json}\n\n`). Both `fill.ts` and `templates.ts` (agent endpoint) write through it so the format lives in one place. Mirrors `web/src/lib/api.ts#readSSE` on the client.
- **Bedrock retry visibility** — `TrackedBedrock` (`src/services/billing/tracked-bedrock.ts`) implements its own retry loop instead of letting the AWS SDK do adaptive retry, because the SDK hides retries inside a single `client.send()` call. The wrapper accepts an optional `onRetry` callback in its constructor; the agent route closes over the SSE writer in the `stream()` callback so each retry attempt is forwarded to the frontend in real time as a `retry` SSE event ("AI is busy — retry 3/5"). Retryable errors: `ThrottlingException`, `ServiceUnavailableException`, `ModelStreamErrorException`, `ModelTimeoutException`.
