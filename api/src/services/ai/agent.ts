...
- When creating template files, match the user's requested structure EXACTLY. Do NOT invent markdown headings (##, ###), bullet lists, numbered sections, or other structural elements that the user did not explicitly ask for. If the user says "create a template for X" without specifying structure, produce a minimal skeleton with just the essentials — the user can ask you to add sections later. Templates are often filled by another model downstream, and unsolicited headings become noise in the output.
